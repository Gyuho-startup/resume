...
/**
 * Warm Professional Template (slug: warm-professional)
 * ATS-friendly: One-column, no tables, no icons, standard headings
 * Amber/warm tones — suited for hospitality, marketing, people roles
 */
export default function WarmProfessionalTemplate({ data, sectionOrder, watermark = false }: TemplateProps) {
  ...
  const sectionMap: Record<ResumeSectionKey, React.ReactNode> = {
    summary: summary ? (
      <section className="mb-6">
        <h2 className="text-sm font-bold uppercase text-amber-800 tracking-wide border-b border-amber-200 pb-1 mb-3">
          Profile
        </h2>
        ...
        <div className="space-y-4">
          {experience.map((exp) => (
            <div key={exp.id} className="page-break-inside-avoid">
              <div className="flex justify-between items-baseline mb-0.5">
                <h3 className="text-sm font-semibold text-amber-900">{exp.position}</h3>
                <span className="text-sm text-slate-500 whitespace-nowrap ml-4">
        ...
        <div className="space-y-4">
          {projects.map((project) => (
            <div key={project.id} className="page-break-inside-avoid">
              <div className="flex justify-between items-baseline mb-0.5">
                <h3 className="text-sm font-semibold text-amber-900">{project.name}</h3>
        ...
        <div className="space-y-2">
          {skills.technical.length > 0 && (
            <p className="text-sm text-slate-700">
              <span className="font-semibold text-amber-900">Technical:</span>{' '}
              {skills.technical.join(', ')}
            </p>
          )}
          {skills.soft.length > 0 && (
            <p className="text-sm text-slate-700">
              <span className="font-semibold text-amber-900">Soft Skills:</span>{' '}
              {skills.soft.join(', ')}
            </p>
          )}
          {skills.languages && skills.languages.length > 0 && (
            <p className="text-sm text-slate-700">
              <span className="font-semibold text-amber-900">Languages:</span>{' '}
          ...
          {certifications.map((cert) => (
            <div key={cert.id} className="page-break-inside-avoid">
              <div className="flex justify-between items-baseline">
                <h3 className="text-sm font-semibold text-amber-900">{cert.name}</h3>
                <span className="text-sm text-slate-500 whitespace-nowrap ml-4">{cert.date}</span>
              </div>
  ...
  return (
    <div className="max-w-[210mm] mx-auto bg-white p-8 shadow-lg relative print:shadow-none">
      ...
      {/* Header — always first */}
      <header className="mb-6 pb-4 border-b-2 border-amber-500">
        <h1 className="text-3xl font-bold text-amber-900 mb-1">{personal.name}</h1>
        <p className="text-sm text-slate-600">
