...
/**
 * Timeline Style Template (slug: timeline-style)
 * ATS-friendly: One-column, no tables, no icons, standard headings
 * Vertical timeline feel with connected left border on each entry.
 */
export default function TimelineStyleTemplate({ data, sectionOrder, watermark = false }: TemplateProps) {
  ...
  const sectionMap: Record<ResumeSectionKey, React.ReactNode> = {
    summary: summary ? (
      <section className="mb-5">
        ...
        <div className="border-l-2 border-slate-300 pl-4 mb-4 relative">
    ...
    education: education.length > 0 ? (
      <section className="mb-5">
        ...
        {education.map((edu) => (
          <div key={edu.id} className="border-l-2 border-slate-300 pl-4 mb-4 relative">
            <span className="text-xs text-slate-500 italic block mb-0.5">
    ...
    experience: experience.length > 0 ? (
      <section className="mb-5">
        ...
        {experience.map((exp) => (
          <div key={exp.id} className="border-l-2 border-slate-300 pl-4 mb-4 relative">
            <span className="text-xs text-slate-500 italic block mb-0.5">
    ...
    projects: projects.length > 0 ? (
      <section className="mb-5">
        ...
        {projects.map((project) => (
          <div key={project.id} className="border-l-2 border-slate-300 pl-4 mb-4 relative">
            {project.startDate && (
              <span className="text-xs text-slate-500 italic block mb-0.5">
    ...
    certifications: certifications && certifications.length > 0 ? (
      <section className="mb-5">
        ...
        {certifications.map((cert) => (
          <div key={cert.id} className="border-l-2 border-slate-300 pl-4 mb-4 relative">
            <span className="text-xs text-slate-500 italic block mb-0.5">{cert.date}</span>
  ...
  return (
    <div className="max-w-[210mm] mx-auto bg-white p-8 shadow-lg relative print:shadow-none">
      ...
      {/* Header — always first */}
      <header className="mb-6 border-l-4 border-slate-400 pl-4">
