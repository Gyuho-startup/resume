...
/**
 * Centered Formal Template (slug: centered-formal)
 * ATS-friendly: One-column, no tables, no icons, standard headings
 * Fully centred header and section titles, formal, elegant.
 * Body text is left-aligned for readability and ATS safety.
 */
export default function CenteredFormalTemplate({ data, sectionOrder, watermark = false }: TemplateProps) {
  ...
  const sectionMap: Record<ResumeSectionKey, React.ReactNode> = {
    summary: summary ? (
      <section className="mb-5">
        <div className="flex items-center gap-3 mb-3">
          ...
          <h2 className="text-xs font-bold uppercase tracking-[0.2em] text-slate-600">Professional Summary</h2>
          <span className="flex-1 border-t border-slate-300" />
        </div>
    ...
    education: education.length > 0 ? (
      <section className="mb-5">
        <div className="flex items-center gap-3 mb-3">
          <span className="flex-1 border-t border-slate-300" />
          <h2 className="text-xs font-bold uppercase tracking-[0.2em] text-slate-600">Education</h2>
          <span className="flex-1 border-t border-slate-300" />
        </div>
    ...
    experience: experience.length > 0 ? (
      <section className="mb-5">
        <div className="flex items-center gap-3 mb-3">
          <span className="flex-1 border-t border-slate-300" />
          <h2 className="text-xs font-bold uppercase tracking-[0.2em] text-slate-600">Experience</h2>
          <span className="flex-1 border-t border-slate-300" />
        </div>
        <div className="space-y-4">
          {experience.map((exp) => (
    ...
    projects: projects.length > 0 ? (
      <section className="mb-5">
        <div className="flex items-center gap-3 mb-3">
          <span className="flex-1 border-t border-slate-300" />
          <h2 className="text-xs font-bold uppercase tracking-[0.2em] text-slate-600">Projects</h2>
          <span className="flex-1 border-t border-slate-300" />
        </div>
        <div className="space-y-4">
    ...
    skills: (skills.technical.length > 0 || skills.soft.length > 0 || (skills.languages && skills.languages.length > 0)) ? (
      <section className="mb-5">
        <div className="flex items-center gap-3 mb-3">
          <span className="flex-1 border-t border-slate-300" />
          <h2 className="text-xs font-bold uppercase tracking-[0.2em] text-slate-600">Skills</h2>
          <span className="flex-1 border-t border-slate-300" />
        </div>
    ...
    certifications: certifications && certifications.length > 0 ? (
      <section className="mb-5">
        <div className="flex items-center gap-3 mb-3">
          <span className="flex-1 border-t border-slate-300" />
          <h2 className="text-xs font-bold uppercase tracking-[0.2em] text-slate-600">Certifications</h2>
          <span className="flex-1 border-t border-slate-300" />
        </div>
        <div className="space-y-2">
          {certifications.map((cert) => (
            <div key={cert.id}>
              ...
              {cert.url && <p className="text-xs text-slate-600">{cert.url}</p>}
            </div>
          ))}
        </div>
      </section>
  ...
  return (
    <div className="max-w-[210mm] mx-auto bg-white p-8 shadow-lg relative print:shadow-none">
      ...
      {/* Header — fully centred, always first */}
      <header className="mb-6 text-center">
        <h1 className="text-3xl font-light text-slate-900 tracking-wide mb-2 uppercase">{personal.name}</h1>
        <div className="border-t border-b border-slate-300 py-1.5 inline-block px-8">
