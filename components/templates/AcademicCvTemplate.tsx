...
/**
 * Academic CV Template (slug: academic-cv)
 * ATS-friendly: One-column, no tables, no icons, standard headings
 * Traditional academic format — education and research prominent
 */
export default function AcademicCvTemplate({ data, sectionOrder, watermark = false }: TemplateProps) {
  ...
  return (
    <div className="max-w-[210mm] mx-auto bg-white p-8 shadow-lg relative print:shadow-none">
      ...
      <header className="mb-6 text-center border-b-2 border-slate-900 pb-4">
        <h1 className="text-2xl font-bold text-slate-900 mb-1">{personal.name}</h1>
        <p className="text-sm text-slate-600">
          {[personal.email, personal.phone, personal.city, personal.linkedin, personal.github].filter(Boolean).join(' | ')}
        </p>
      </header>
