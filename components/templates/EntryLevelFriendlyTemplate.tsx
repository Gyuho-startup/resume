...
/**
 * Entry Level Friendly Template (slug: entry-level-friendly)
 * ATS-friendly: One-column, no tables, no icons, standard headings
 * Optimised for fresh graduates — summary/profile prominent, teal accent.
 * Summary shown in a teal highlight box directly below header — not part of sectionOrder.
 */
export default function EntryLevelFriendlyTemplate({ data, sectionOrder, watermark = false }: TemplateProps) {
  ...
  const sectionMap: Record<ResumeSectionKey, React.ReactNode> = {
    // Summary is rendered in a teal box directly below the header in this template
    summary: null,

    education: education.length > 0 ? (
      <section className="mb-6">
        <h2 className="text-base font-bold text-teal-700 border-b-2 border-teal-200 pb-1 mb-3">
          Education
        </h2>
  ...
  return (
    <div className="max-w-[210mm] mx-auto bg-white p-8 shadow-lg relative print:shadow-none">
      ...
      <header className="mb-6 pb-4 border-b-2 border-teal-500">
        <h1 className="text-3xl font-bold text-teal-800 mb-1">{personal.name}</h1>
        <p className="text-sm text-slate-600">
          {[personal.email, personal.phone, personal.city, personal.linkedin, personal.github, personal.portfolio].filter(Boolean).join(' | ')}
        </p>
      </header>

      {/* Summary — always shown at top with special teal box treatment */}
      {summary && (
        <div className="bg-teal-50 border border-teal-100 rounded-sm px-4 py-3 mb-6">
