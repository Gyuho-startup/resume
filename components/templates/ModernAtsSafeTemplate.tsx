 ...
 * Blue accent underline on name, coloured section headings with bottom rule,
 * entry items with subtle left border — Dublin-style modern template
 * Ideal for a balance between modern aesthetics and ATS compliance
 */
export default function ModernAtsSafeTemplate({ data, sectionOrder, watermark = false }: TemplateProps) {
  ...
  const sectionMap: Record<ResumeSectionKey, React.ReactNode> = {
    summary: summary ? (
      <section className="mb-6">
        <h2 className="text-base font-bold text-blue-700 border-b border-blue-200 pb-1 mb-3">
          Summary
        </h2>
        <div className="border-l-2 border-slate-200 pl-3 mb-4">
    ...
    education: education.length > 0 ? (
      <section className="mb-6">
        ...
        <div className="space-y-4">
          {education.map((edu) => (
            <div key={edu.id} className="border-l-2 border-slate-200 pl-3 mb-4">
      ...
      <section className="mb-6">
        ...
        <div className="space-y-4">
          {projects.map((project) => (
            <div key={project.id} className="border-l-2 border-slate-200 pl-3 mb-4">
              <div className="flex justify-between items-baseline mb-0.5">
    ...
    experience: experience.length > 0 ? (
      <section className="mb-6">
        ...
        <div className="space-y-4">
          {experience.map((exp) => (
            <div key={exp.id} className="border-l-2 border-slate-200 pl-3 mb-4">
              <div className="flex justify-between items-baseline mb-0.5">
    ...
    skills: (skills.technical.length > 0 || skills.soft.length > 0 || (skills.languages && skills.languages.length > 0)) ? (
      <section className="mb-6">
        ...
        <div className="border-l-2 border-slate-200 pl-3 mb-4 space-y-2">
          {skills.technical.length > 0 && (
            <div>
              <span className="text-sm font-semibold text-slate-900">Technical: </span>
              <span className="text-sm text-slate-700">{skills.technical.join(', ')}</span>
            </div>
          )}
          {skills.soft.length > 0 && (
            <div>
              <span className="text-sm font-semibold text-slate-900">Soft Skills: </span>
      ...
      <section className="mb-6">
        ...
        <div className="space-y-2">
          {certifications.map((cert) => (
            <div key={cert.id} className="border-l-2 border-slate-200 pl-3 mb-4">
  ...
  return (
    <div className="max-w-[210mm] mx-auto bg-white p-8 shadow-lg relative print:shadow-none">
      ...
      {/* Personal Information (Header) — always first */}
